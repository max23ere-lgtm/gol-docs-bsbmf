...
function App() {
  ...
  // AUTO-SAVE: Só dispara se o carregamento inicial foi concluído com sucesso
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 flex flex-col font-sans transition-colors duration-500 text-gray-800 dark:text-gray-100">
      ...
      <header className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md sticky top-0 z-40 border-b border-gray-200 dark:border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div className="hidden xs:block border-l pl-4 border-gray-200 dark:border-zinc-700">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                 ...
                 <span className={`text-[10px] font-bold uppercase tracking-wide ${syncError ? 'text-red-500' : 'text-gray-400 dark:text-zinc-500'}`}>
                   {isSyncing ? 'Salvando...' : syncError ? 'Erro de Conexão' : 'Dados Sincronizados'}
                 </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowStatsModal(true)} className="hidden sm:flex items-center gap-2 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-bold transition-all shadow-md">
               <PieChart className="w-4 h-4" />
               Relatórios
            </button>
            <button onClick={() => setDarkMode(!darkMode)} className="p-2.5 rounded-xl text-gray-500 hover:bg-gray-100 dark:text-zinc-400 dark:hover:bg-zinc-800 transition-all border dark:border-zinc-700">
               {darkMode ? <Sun className="w-5 h-5 text-yellow-500" /> : <Moon className="w-5 h-5" />}
            </button>
            <button onClick={() => loadData()} className="flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:text-zinc-300 rounded-xl text-xs font-bold transition-all border dark:border-zinc-700">
      ...
      {syncError && (
        <div className="bg-red-500 text-white text-[10px] font-bold py-1 px-4 text-center animate-pulse">
           ⚠️ FALHA NA SINCRONIZAÇÃO COM A NUVEM. Verifique sua internet ou permissões de rede.
        </div>
      )}
            ...
            <div className="flex gap-3">
              <div className="relative flex-1 group">
                ...
                <input 
                  ...
                  placeholder="Inserir Manual ou Scanner" 
                />
              </div>
              <button 
                onClick={startCamera}
                className="px-5 bg-gray-900 dark:bg-zinc-700 text-white rounded-2xl hover:bg-orange-600 transition-colors flex items-center justify-center"
              >
                    ...
                    <tr key={doc.id} className="hover:bg-orange-50/30 dark:hover:bg-zinc-800/50 transition-colors group flex flex-col md:table-row">
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className={`flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center font-bold text-[10px] ${doc.type === 'RTA' ? 'bg-blue-600 text-white' : 'bg-indigo-600 text-white'}`}>
                            {doc.type}
                          </div>
                          <div className="ml-4">
                            <div className="text-sm font-bold text-gray-900 dark:text-white font-mono">{doc.id}</div>
                            <div className="text-[10px] font-medium text-gray-400">{new Date(doc.createdAt).toLocaleDateString()}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-5 whitespace-nowrap">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        {doc.errorCount > 0 ? (
                          <div className="flex items-center gap-1 text-red-500 text-xs font-bold"><AlertCircle className="w-3.5 h-3.5" /> {doc.errorCount}</div>
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-2 flex-wrap">
                           ...
                           {doc.status === DocStatus.RETURN && (
                             <button onClick={() => { toggleError(doc.id); updateStatus(doc.id, DocStatus.CONFERENCE); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 text-white rounded-lg text-[10px] font-bold uppercase"><CheckCircle2 className="w-3 h-3" /> Corrigido</button>
                           )}
                           {doc.status === DocStatus.SCANNER && (
                             <button onClick={() => updateStatus(doc.id, DocStatus.ACCEPTANCE)} className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-lg text-[10px] font-bold uppercase">Digitalizado <ArrowRight className="w-3 h-3" /></button>
                           )}
                           {doc.status === DocStatus.ACCEPTANCE && (
                             <button onClick={() => updateStatus(doc.id, DocStatus.SHIPPING)} className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500 text-white rounded-lg text-[10px] font-bold uppercase">Aceitar <ArrowRight className="w-3 h-3" /></button>
                           )}
                           {doc.status === DocStatus.SHIPPING && (
                             <button onClick={() => updateStatus(doc.id, DocStatus.COMPLETED)} className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-600 text-white rounded-lg text-[10px] font-bold uppercase">Finalizar <CheckCircle2 className="w-3 h-3" /></button>
                           )}
      ...
      <footer className="border-t border-gray-200 dark:border-zinc-800 py-8 mt-8 bg-white dark:bg-zinc-900 transition-colors">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 dark:text-zinc-500 gap-4">
          ...
          <div className="flex items-center gap-1 bg-gray-50 dark:bg-zinc-800 px-3 py-1.5 rounded-full border dark:border-zinc-700">
            Desenvolvido por <strong className="text-orange-600 dark:text-orange-500">José Augusto Torres</strong> • <a href="mailto:jatgsilva@voegol.com.br" className="hover:text-orange-500 underline">jatgsilva@voegol.com.br</a>
          </div>
        </div>
      </footer>
      ...
      {showCamera && (
        <div className="fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center">
          <div className="absolute top-6 right-6 z-50">
            <button onClick={stopCamera} className="bg-zinc-800/80 p-3 rounded-full text-white"><X className="w-6 h-6" /></button>
          </div>
          <div className="w-full h-full relative flex items-center justify-center">
            ...
            {isCameraLoading && (
              <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-50">
                ...
                <p className="text-white font-bold text-2xl">IA Analisando...</p>
              </div>
            )}
          </div>
          <div className="absolute bottom-12 w-full flex flex-col items-center gap-6 z-50 px-6">
            ...
            <button onClick={captureAndAnalyze} disabled={isCameraLoading} className="w-20 h-20 rounded-full bg-white border-4 border-zinc-200 flex items-center justify-center shadow-2xl active:scale-95 group">
              <div className="w-16 h-16 bg-orange-600 rounded-full flex items-center justify-center"><Camera className="w-8 h-8 text-white" /></div>
            </button>
          </div>
        </div>
      )}
